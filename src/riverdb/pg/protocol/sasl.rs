...
//! MIT licensed code from https://github.com/sfackler/rust-postgres.
...
pub(crate) fn hi(str: &[u8], salt: &[u8], i: u32) -> [u8; 32] {
    ...
    let mut prev_result = [0u8; 32];
    let mut hmac = Hmac::new(Sha256::new(), str);
    hmac.input(salt);
    hmac.input(&[0, 0, 0, 1]);
    ...
    for _ in 1..i {
        let mut hmac = Hmac::new(Sha256::new(), str);
        hmac.input(&prev);
        hmac.raw_result(&mut prev_result[..]);
        prev = &prev_result[..];

        for (hi, prev) in (&mut result[..]).iter_mut().zip(prev) {
            *hi ^= prev;
        }
    }
...
impl ScramSha256 {
    ...
    pub fn update(&mut self, message: &[u8]) -> io::Result<()> {
        ...
        let mut hmac = Hmac::new(Sha256::new(),&salted_password);
        hmac.input(b"Client Key");
        let mut client_key = [0u8; 32];
        hmac.raw_result(&mut client_key[..]);

        let mut hash = Sha256::new();
        hash.input(&client_key[..]);
        let mut stored_key = [0u8; 32];
        hash.result(&mut stored_key[..]);
        ...
        let mut hmac = Hmac::new(Sha256::new(), &stored_key[..]);
        hmac.input(auth_message.as_bytes());
        let client_signature = hmac.result();

        for (proof, signature) in (&mut client_key[..]).iter_mut().zip(client_signature.code()) {
            *proof ^= signature;
        }

        write!(&mut self.message, ",p={}", base64::encode(&client_key[..])).unwrap();
    ...
    pub fn finish(&mut self, message: &[u8]) -> io::Result<()> {
        ...
        let mut hmac = Hmac::new(Sha256::new(), server_key.code());
        hmac.input(auth_message.as_bytes());
        let verifier_mac = MacResult::new_from_owned(verifier);
        if hmac.result().eq(&verifier_mac) {
            Ok(())
        } else {
