...
pub fn register_client_connect_plugin(plugin: &'static (dyn ClientConnectPlugin + Sync + Send)) {
    unsafe {
        CLIENT_CONNECT_PLUGINS.push(plugin);
    }
}

static mut CLIENT_CONNECT_PLUGINS: Vec<&'static (dyn ClientConnectPlugin + Sync + Send)> = Vec::new();

pub struct ClientConnectContext {
    plugins: &'static [&'static (dyn ClientConnectPlugin + Sync + Send)],
    index: usize,
}
...
impl ClientConnectContext {
    ...
    pub async fn next(&mut self, client: &mut ClientConn) -> Result<&'static PostgresCluster> {
        let i = self.index;
...
pub trait ClientConnectPlugin {
    async fn client_connected(&self, ctx: &mut ClientConnectContext, client: &mut ClientConn) -> Result<&'static PostgresCluster>;
}

pub async fn run_client_connect_plugins(client: &mut ClientConn) -> Result<&'static PostgresCluster> {
